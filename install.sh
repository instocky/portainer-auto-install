...
verify_installation() {
    ...
    echo "   🌐 HTTP:  http://$LOCAL_IP:9000 (рекомендуется)"
    echo "   🔒 HTTPS: https://$LOCAL_IP:9443 (самоподписанный сертификат)"
    if [[ "$PUBLIC_IP" != "недоступен" ]]; then
        echo "   🌍 Внешний HTTP:  http://$PUBLIC_IP:9000"
        echo "   🌍 Внешний HTTPS: https://$PUBLIC_IP:9443 (требует подтверждения)"
    fi
    echo ""
    echo "⚠️  Для HTTPS: браузер покажет предупреждение безопасности"
